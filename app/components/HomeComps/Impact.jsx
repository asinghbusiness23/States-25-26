...
const Impact = () => {
  ...
  return (
    <section ref={ref} className="bg-[#d3efca] px-20 py-24 text-white">
      <motion.div
      ...
      >
        ...
        >
          Strengthening Chester County, Together
        </motion.h2>
        <motion.p variants={fadeUp} className="text-[#1B6E4F] mt-3 text-base">
          Every connection made is a family supported.
        </motion.p>
      </motion.div>
      ...
      >
        {[
          ...
          { icon: <SunIco />, n: 50, suffix: "", label: "Active Volunteers" },
          { icon: <HeartIco />, n: 4, suffix: " yrs", label: "Years Serving" },
        ].map(({ icon, n, suffix, label }) => (
          <motion.div
            key={label}
            variants={fadeUp}
            whileHover={{ scale: 1.04 }}
            className="text-center bg-white rounded-2xl py-10 px-6 border border-white/10 backdrop-blur-sm"
          >
            <div className="text-[#0cc883] flex justify-center mb-4">
              {icon}
            </div>
            <p className="text-5xl font-black text-[#264653] leading-none">
              <CountUp end={n} suffix={suffix} />
            </p>
            <p className="text-[#264653] text-xs font-semibold mt-2 tracking-wide">
              {label}
            </p>
          </motion.div>
        ))}
