...
const Testimonials = () => {
  ...
  return (
    <section
      ref={ref}
      className="bg-[#f8faf8] px-4 sm:px-8 md:px-20 py-14 sm:py-24"
    >
      <motion.div
        variants={stagger(0.1)}
        initial="hidden"
      ...
      {/* 1-col on mobile, 3-col on lg */}
      <motion.div
        variants={stagger(0.13)}
        initial="hidden"
        animate={inView ? "show" : "hidden"}
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-7 max-w-6xl mx-auto"
      >
        {testimonials.map((t, i) => (
          <motion.div
            key={i}
            ...
            className="bg-white rounded-2xl p-6 sm:p-8 border border-[#d3efca] shadow-sm transition-shadow"
          >
            <div className="text-[#0cc883] mb-4 sm:mb-5">
              <QuoteIco />
            </div>
            <p className="text-[#264653] text-[14px] sm:text-[15px] leading-[1.8] font-medium italic mb-5 sm:mb-7">
              "{t.quote}"
            </p>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-[#1B6E4F] to-[#0cc883] flex items-center justify-center text-white font-black text-sm shrink-0">
                {t.avatar}
              </div>
              <div>
                <p className="font-bold text-[#264653] text-sm">{t.name}</p>
